...
`;
  ...
  async publishCurrentNote(view) {
    ...
    try {
      ...
      const currentFileName = ((_b = view.file) == null ? void 0 : _b.basename) || "Untitled Note";
    ...
    try {
      ...
      if (!targetDir) {
        try {
          ...
          new import_obsidian2.Notice(`Failed to create directory: ${targetDirPath}`);
          return false;
        }
      }
      if (!targetDir || targetDir instanceof import_obsidian2.TFile) {
        new import_obsidian2.Notice(`${targetDirPath} is not a directory`);
        return false;
      }
      const newPath = `${targetDirPath}/${file.name}`;
      if (this.app.vault.getAbstractFileByPath(newPath)) {
        new import_obsidian2.Notice(`A file named ${file.name} already exists in ${targetDirPath}`);
        return false;
      }
      await this.app.fileManager.renameFile(file, newPath);
      new import_obsidian2.Notice(`Moved ${file.name} to ${targetDirPath}`);
      return true;
    } catch (error) {
      console.error("Error moving note to published directory:", error);
      new import_obsidian2.Notice(`Error moving note: ${error}`);
      return false;
    }
  }
};
var GhostyPostySettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("Move Notes After Publishing").setDesc("Move notes to a specified directory after they are published").addToggle((toggle) => toggle.setValue(this.plugin.settings.moveNotesAfterPublish).onChange(async (value) => {
      this.plugin.settings.moveNotesAfterPublish = value;
      await this.plugin.saveSettings();
      publishedDirSetting.settingEl.style.display = value ? "flex" : "none";
    }));
    const publishedDirSetting = new import_obsidian2.Setting(containerEl).setName("Published Notes Directory").setDesc("Directory where published notes will be moved to").addText((text) => text.setPlaceholder("Published Notes").setValue(this.plugin.settings.publishedNotesDirectory).onChange(async (value) => {
